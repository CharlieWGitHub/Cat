...
-(void)event{
    ...
    self.lockView.setPWTwiceSameBlock = ^(NSString *pwd){
        ...
        if(self->_successBlock != nil) self->_successBlock(self,pwd);
        
        if(CoreLockTypeModifyPwd == self->_type){
    ...
    self.lockView.verifyPwdBlock = ^(NSString *pwd){
        ...
        if(res){//密码一致
            ...
            if(CoreLockTypeVeryfiPwd == self->_type){
                
                //禁用交互
                self.view.userInteractionEnabled = NO;
                
            }else if (CoreLockTypeModifyPwd == self->_type){//修改密码
