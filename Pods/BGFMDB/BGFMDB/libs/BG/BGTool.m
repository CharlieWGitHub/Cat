...
+ (NSString *)getSqlType:(NSString *)type
{
    if ([type isEqualToString:@"i"] || [type isEqualToString:@"I"] ||
        ...
        [type isEqualToString:@"c"] || [type isEqualToString:@"C"] | [type isEqualToString:@"l"] || [type isEqualToString:@"L"]) {
        return SqlInteger;
    }
...
+ (BOOL)ifNotExistWillCreateTableWithObject:(id)object ignoredKeys:(NSArray *const)ignoredKeys
{
    ...
    [[BGDB shareManager] isExistWithTableName:tableName
                                     complete:^(BOOL isExist) {
                                       if (!isExist) { //如果不存在就新建
                                           ...
                                           [[BGDB shareManager] createTableWithTableName:tableName
                                                                                    keys:createKeys
